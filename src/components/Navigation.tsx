        ...
        <div className="flex items-center justify-between h-20">
          ...
          <Link href="/" className="flex items-center space-x-3">
            <Image
              ...
              width={48}
              height={48}
              className="rounded-full shadow-golf object-contain bg-white/0"
              priority
            />
            <div className="flex flex-col">
              <span className={`font-serif text-2xl font-bold transition-colors leading-tight ${
          ...
          <div className="flex items-center space-x-4">
            {/* Cart */}
            <Link href="/cart" className="relative p-2 rounded-full hover:bg-golf-gold-400/20 transition-all duration-300 group">
              <ShoppingCart className={`w-6 h-6 transition-colors duration-300 ${
                isScrolled ? 'text-golf-green-900 group-hover:text-golf-gold-400' : 'text-white group-hover:text-golf-gold-400'
              }`} />
              {totalItems > 0 && (
                <motion.span
                  initial={{ scale: 0 }}
                  animate={{ scale: 1 }}
                  className="absolute -top-1 -right-1 bg-golf-gold-400 text-golf-green-900 text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center shadow-lg"
                >
            ...
            <div className="px-4 py-6 space-y-4">
              {navItems.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
                  className="block text-golf-green-900 font-medium hover:text-golf-gold-500 transition-colors"
